/**
 * Discord Notifier — Sends webhook messages with embeds
 */

import type { HealingNotification, SelfHealingConfig, Severity } from "../types";

/**
 * Send a healing notification to Discord via webhook.
 */
...
function buildDiscordEmbed(notification: HealingNotification): Record<string, unknown> {
  ...
  if (testResult) {
    fields.push({
      name: "Tests",
      value: testResult.passed ? "\u2705 Passed" : "\u274C Failed",
      inline: true,
    });
  }
...
function severityToColor(severity: Severity): number {
  switch (severity) {
    case "critical": return 0xff0000;
    case "error": return 0xff8c00;
